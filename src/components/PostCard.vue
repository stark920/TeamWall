...
<template>
  <div class="rounded-lg border-2 border-black bg-white p-6">
    ...
    <div class="flex items-center mb-5">
      <AvatarVue class="mx-2" size="40" :imgUrl="'https://i.pravatar.cc/150?img=19'" />
      <div class="flex w-full">
        <input class="border-2 border-black w-full" type="text" />
        <button
          class="border-2 border-black text-base text-white bg-primary inline-block w-full max-w-[128px]">留言</button>
      </div>
    </div>
    <div class="bg-secondary rounded-lg px-4 py-5 mb-4" v-for="(comment, index) in post.comments" :key="index">
      <UserInfo class="mb-4" :imgUrl="comment.avatar" :name="comment.userName" userPageUrl="#"
