<template>
  <div class="mb-4 grid gap-x-3 gap-y-1.5 md:grid-cols-3">
    <div>
      <select class="h-11 w-full border-2 border-black" name="category" v-model.number="sort" @change="getPosts">
    ...
    <div class="md:col-span-2">
      <div class="flex h-11 border-2 border-black">
        <input class="w-full border-0" type="text" placeholder="搜尋貼文" v-model.trim="searchKey"
          @keyup.enter="getPosts" />
        <button type="button" class="flex w-11 items-center justify-center border-l-2 border-black bg-primary"
          @click="getPosts">
