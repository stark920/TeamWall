<script setup>
import { RouterLink } from "vue-router";
import AvatarVue from "../components/Avatar.vue";
import IconButtonVue from "../components/IconButton.vue";
import IconBellVue from "../components/icons/IconBell.vue";
import IconThumbsUpVue from "../components/icons/IconThumbsUp.vue";
import IconChatVue from "../components/icons/IconChat.vue";
defineProps({
  imgUrl: {
    type: String,
  },
  name: {
    type: String,
    default: "Unknown",
  },
  userPageUrl: {
    type: String,
    default: "#",
  },
});
</script>

<template>
  <div class="bg-white flex flex-col px-6 py-8 border-2 border-black">
    <RouterLink
      to="/post-news"
      class="flex items-center justify-center text-lg py-2 bg-primary text-white border-2 border-black rounded-lg mb-6 hover:bg-warning hover:text-black"
    >
      張貼動態
    </RouterLink>

    <ul class="flex flex-col ml-2">
      <li>
        <RouterLink to="#" class="flex items-center mb-5">
          <AvatarVue size="50" :imgUrl="imgUrl" class="cursor-pointer" />
          <h3 class="font-bold ml-4">{{ name }}</h3>
        </RouterLink>
      </li>
      <li>
        <RouterLink to="#" class="flex items-center mb-5">
          <IconButtonVue size="50">
            <IconBellVue class="w-6 h-6" />
          </IconButtonVue>
          <h3 class="font-bold ml-4">追蹤名單</h3>
        </RouterLink>
      </li>
      <li>
        <RouterLink to="#" class="flex items-center mb-5">
          <IconButtonVue size="50">
            <IconThumbsUpVue class="w-6 h-6" />
          </IconButtonVue>
          <h3 class="font-bold ml-4">我按讚的文章</h3>
        </RouterLink>
      </li>
      <li>
        <RouterLink to="/chatroomlist" class="flex items-center">
          <IconButtonVue size="50">
            <IconChatVue class="w-6 h-6" />
          </IconButtonVue>
          <h3 class="font-bold ml-4">聊天室</h3>
        </RouterLink>
      </li>
    </ul>
  </div>
</template>
